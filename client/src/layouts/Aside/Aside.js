...
const Aside = () => {
	// ── 1. Read state directly from the Orchestrator ──
	...
	// Apply collapsed state immediately on render
   ...
   `;
	...
	requestAnimationFrame(() => {
		...
		// 1. Grab the app container so we can update the grid immediately
		...
		const updateSidebarAuthContent = () => {
			...
			if (navContentHost) navContentHost.innerHTML = generateMenuContent();
		...
		// Clean up listeners when sidebar is unmounted
